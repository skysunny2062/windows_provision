...
1. [cite_start]雙擊執行 `windows_provision.bat`。腳本內建 UAC 提權機制，會自動要求以**系統管理員**身份執行 [cite: 6]。
2. [cite_start]腳本會檢查系統環境。若未安裝 Python，將自動透過 winget 靜默安裝 **Python 3.14** [cite: 4]。
3. [cite_start]環境確認無誤後，腳本會自動呼叫並執行 `core\main.py` [cite: 3]。
4. 主選單出現後選擇所需模式：
   - **1. 系統部署**：進入安裝與還原問卷流程。
   - **2. 資料備份**：備份當前使用者資料夾。
   - **3. 結束**
